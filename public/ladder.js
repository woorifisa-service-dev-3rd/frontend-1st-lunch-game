let numberOfLadders = 2;
let lad = ladder('canvas_ladder');
createInputForms(numberOfLadders);

document.addEventListener('DOMContentLoaded', () => {
    const ladderSelect = document.getElementById('ladders');
    ladderSelect.addEventListener('change', () => {
        numberOfLadders = parseInt(ladderSelect.value) + 1;
        for (let i = 1; i <= 6; i++) {
        ...
        // numberOfLadders가 변경될 때마다 ladder 함수를 호출합니다.
        lad = ladder('canvas_ladder', { line: numberOfLadders });
        createInputForms(numberOfLadders);
    });

    // 초기 버튼 개수 설정
    ladderSelect.dispatchEvent(new Event('change'));
});

// 전체 초기화
const share = document.getElementById('again');
share.addEventListener('click', () => {
    lad = ladder('canvas_ladder', { line: numberOfLadders });
    createInputForms(numberOfLadders);
});

function createInputForms(count) {
    const container = document.getElementById('inputFormsContainer');
    container.innerHTML = ''; // Clear existing forms

    const canvas_ladder = document.getElementById('canvas_ladder');
    const canvasWidth = canvas_ladder.width;
    const formWidth = (canvasWidth / count) * 0.6;
    ...
    for (let i = 0; i < count; i++) {
        ...
        input.classList.add('btn_ladder');

        input.style.width = formWidth + 'px';
        input.style.left = (formWidth * i) + 'px';
...
function ladder(_id, option = {}) {
    const canvas_ladder = document.getElementById(_id);
    let ctx = canvas_ladder.getContext('2d');
    let width = canvas_ladder.width;
    let height = canvas_ladder.height;

    let line = option.line || numberOfLadders; // line
    ...
    // 마우스가 움직일 때 이벤트 입니다
    canvas_ladder.addEventListener('mousemove', (event) => {
        if (!ctx) return;
    ...
    // 마우스 다운 이벤트
    canvas_ladder.addEventListener('mousedown', (event) => {
        if (!ctx) return;
        isClicked = true;
        let x1 = event.offsetX;
        let y1 = event.offsetY;
        if (isClicked) {
            ...
            startBridge =
                { ...startTarget, x: startTarget.object.x, y: y1, originX: x1, originY: y1 };  // x축은 그려진 선 기준값을 대입 합니다
        }
    });

    // 마우스 업 이벤트
    canvas_ladder.addEventListener('mouseup', (event) => {
        if (!ctx) return;
        // hoverPosition = {};
        if (isClicked) {  // 마우스가 다운된 상태의 조건이 충족하면,
            ...
            if (startBridge.dataIndex == endBridge.dataIndex
                || Math.abs(startBridge.dataIndex - endBridge.dataIndex) > 1) {
                isClicked = false;
                // hoverPosition = {};
                startBridge = null;
                _init();
    ...
    // 마우스 아웃 이벤트
    canvas_ladder.addEventListener('mouseleave', () => {
        if (!ctx) return;
        isClicked = false;
        // hoverPosition = {};
    });
